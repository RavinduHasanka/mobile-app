...
class AccessoriesProductsScreen extends StatefulWidget {
  ...
  const AccessoriesProductsScreen({
    Key? key
  }) : super(key: key);

  @override
  _AccessoriesProductsScreenState createState() => _AccessoriesProductsScreenState();
}
        ...
        child: Padding(
          ...
          child: FutureBuilder<Products?>(
            ...
            builder: (context, snapshot) {
              ...
              // Check if there are any products category name "Accessories"

              bool hasAccessoriesProducts = productdata?.existProducts?.any((product) => product.categories?.first.categoryName == 'accessories' || product.conditions == 'New' ) ?? false;

              if (!hasAccessoriesProducts) {
                return Center(
                  child: Text(
              ...
              return GridView.builder(
                ...
                itemBuilder: (context, index) {
                  // Filter products by category name "Accessories"
                  if (productdata?.existProducts?.elementAt(index).categories?.first.categoryName == "Accessories" ||
                      productdata?.existProducts?.elementAt(index).categories?.first.categoryName == "accessories") {
                    return SizedBox(
                      width: 140,
